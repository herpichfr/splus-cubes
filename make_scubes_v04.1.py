...
class SCubes(object):
    def __init__(self):
        self.code = 'make_scubes'
        self.lastversion = '0.4.1'
        self.moddate = '2021-04-28'
        ...
        self.wave_eff = {"F378": 3770.0, "F395": 3940.0, "F410": 4094.0,
                         ...
                         "U": 3536.0, "Z": 8831.0}
        self.delta_wave = {"F378": 151.0, "F395": 103.0, "F410": 201.0,
                          "F430": 201.0, "F515": 207.0, "F660": 147.0,
                          "F861": 408.0, "G": 1545.0, "I": 1506.0, "R": 1465.0,
                          "U": 352.0, "Z": 1182.0}
    ...
    def make_det_stamp(self, savestamp: bool=True):
        ...
        for field in fields:
            ...
            for i, (name, size) in enumerate(zip(fnames, fsizes)):
                for imtype in ['det_scimas', 'det_wimas']:
                    galdir = os.path.join(outdir, name)
                    if not os.path.isdir(galdir):
                        os.makedirs(galdir)
                    doutput = os.path.join(galdir,
                                           "{0}_{1}_{2}x{2}_{3}.fits".format(
                                               name, field_name, size, imtype))
                    if not os.path.isfile(doutput):
                        try:
                            d = fits.open(os.path.join(tile_dir, field_name + '_' + imtype + '.fits'))[0]
                        except:
                            d = fits.open(os.path.join(tile_dir, field_name + '_' + imtype + '.fits.fz'))[1]
                        dheader = d.header
                        ddata = d.data
                        wcs = WCS(dheader)
                        xys = wcs.all_world2pix(fcoords.ra, fcoords.dec, 1)
    ...
    def run_sex(self, pathdetec: str=None, galaxy: str=None, tile: str=None, size: int=None):
        ...
        pathdetect = os.path.join(galdir, "{0}_{1}_{2}x{2}_{3}.fits".format(
            galaxy, tile, size, 'det_scimas')) if pathdetec is None else pathdetec
    ...
    def make_Lupton_colorstamp(self, galaxy: str=None, tile: str=None,
                               size: int=None):
    ...
    def ensable_stamps(self, galdir: str=None, bands: list=None,
                       specz="", photz="", bscale: float=1e-19):
        """ group stamps for both data and erros within hdus"""
        ...
        if has_errs:
            ...
            dataerr = np.sqrt(1.0 / np.clip(weights, 1, np.infty) + np.clip(data, 0, np.infty) / gain[:, None, None])
    ...
    def calc_masks(self, galaxy: str=None, tile: str=None, size: int=None,
                   savemask: bool=False, bands: list=None,
        ...
        size = self.sizes[0] if size is None else size
        bands = self.bands if bands is None else bands
        angsize = self.angsize if angsize is None else angsize
        galdir = os.path.join(outdir, galaxy)
        ...
        # fdata = ddata.copy()
        sigThresh = abs(np.percentile(fdata, 16))
        # constraints = fdata > sigThresh # will need this for latter
        # calculate the best big circle
        ...
        while increaseCircle:
            bigCircRadius = angsize * multFact
            circregion = CirclePixelRegion(center=PixCoord(centralPixCoords[0],
                                                           centralPixCoords[1]),
                                           radius=bigCircRadius)
            multFact += 0.05
            outerCircRadius = angsize * multFact
            if outerCircRadius >= size / 2:
                raise StopIteration('Circle size exceeds stamp size')
            outerCircle = CirclePixelRegion(center=PixCoord(centralPixCoords[0],
                                                            centralPixCoords[1]),
                                            radius=outerCircRadius)
            maskAnulus = (distance > bigCircRadius) & (distance < outerCircRadius)
            print('multFact:', multFact, '\n',
                  'bigRadius:', bigCircRadius, '\n',
                  'outerRadius:', outerCircRadius, '\n',
                  'percentile:', abs(np.percentile(fdata, 16)))
            print('inner\n',
                  'mean:', np.mean(fdata[distance < bigCircRadius]), '\n',
                  'median:', np.median(fdata[distance < bigCircRadius]), '\n',
                  'std:', np.std(fdata[distance < bigCircRadius]))
            print('annulus\n',
                  'mean:', np.mean(fdata[maskAnulus]), '\n',
                  'median:', np.median(fdata[maskAnulus]), '\n',
                  'std:', np.std(fdata[maskAnulus]))
            if np.median(fdata[maskAnulus]) <= sigThresh:
                increaseCircle = False
        # return 'leaving'

        # creating mask1 constraining the big circle
        print('creating mask1 constraining the big circle')
        mask1 = np.zeros(fdata.shape)
        mask1[distance > bigCircRadius] = 1
        # fdata *= mask1

        sewcat = self.run_sex(pathdetec=pathdetect, galaxy=galaxy, tile=tile, size=size)
        sewpos = np.transpose((sewcat['table']['X_IMAGE'], sewcat['table']['Y_IMAGE']))
        radius = 2.0 * (sewcat['table']['FWHM_IMAGE'] / 0.55)
        ...
        mask2 = np.zeros(fdata.shape)
        maskeddata[distance > bigCircRadius] = 0
        circregion.plot(color='y', lw=1.5)
        ...
        # get SN mask
        hdus, tab = self.ensable_stamps(galdir=galdir, bands=bands)
        mask4 = np.zeros(fdata.shape)
        for flux, flux_err in zip(hdus[1].data, hdus[2].data):
            mask4[flux / flux_err <= 2] = 4

        ax4 = plt.subplot(224, projection=wcs)
        fitsmask = f.copy()
        fitsmask[1].data = mask1 + mask2 + mask4  # np.zeros(maskeddata.shape)
        ax4.imshow(fitsmask[1].data, cmap='Greys', origin='lower')
        ax4.set_title('Mask')
        ax4.set_xlabel('RA')
        ax4.set_ylabel('Dec')
        ...
        fitsmask[1].header['X0STAMP'] = (centralPixCoords[0].item(), '[pixel] Central position on stamp')
        fitsmask[1].header['Y0STAMP'] = (centralPixCoords[1].item(), '[pixel] Central position on stamp')
        fitsmask[1].header['CRASTAMP'] = (galcoords.ra.value, '[deg] Central position on stamp (Simbad)')
        fitsmask[1].header['CDESTAMP'] = (galcoords.dec.value, '[deg] Central position on stamp (Simbad)')
        fitsmask[1].header['ANGSIZE'] = (angsize, '[arcsec] angular size of central galaxy (Simbad)')
        fitsmask[1].header['MULTFAC'] = (multFact, '[pixel/arcsec] multiplicative factor over angsize')
        fitsmask[1].header['CLASS_STAR'] = (self.class_star, 'Value of SExtractor CLASS_STAR used for detection')
    ...
    def calculate_galphot(self, hdus, fitsmask, tab, galaxy: str=None,
                          tile: str=None, size: float=None, bands: list=None,
                          angsize: float=None):
        """Calculate the galaxy photometry using the mask and aperture radii
        defined within calc_masks"""
        galcoords = SkyCoord(ra=self.coords[0][0], dec=self.coords[0][1],
                             unit=(u.hour, u.deg))
        outdir = os.getcwd() if self.work_dir is None else self.work_dir
        galaxy = self.galaxies[0] if galaxy is None else galaxy
        tile = self.tiles[0] if tile is None else tile
        ...
        indexes = [1, 2, 3, 5, 6, 7]
        mask = np.ones(fitsmask[1].data.shape)
        for i in indexes:
            mask[fitsmask[1].data == i] = 0

        # kernel = Gaussian2DKernel(x_stddev=2)
        # kernel = Gaussian2DKernel(x_stddev=2, x_size=51, y_size=51)
        # astropy_convs = [convolve(img*mask, kernel) for img in hdus[1].data]
        # astropy_errorconvs = [convolve(img*mask, kernel) for img in hdus[2].data]
        # indexes = [1, 3, 5, 7]
        # extmask = np.ones(fitsmask[1].data.shape)
        # for i in indexes:
        #     extmask[fitsmask[1].data == i] = 0
        # aconvfluxes = np.array([np.sum(aconv * mask) for aconv in astropy_convs])
        # aconvfnu = 3.34e4 * tab['WAVE_EFF']**2 * aconvfluxes * 1e-19
        # aconv_magAB = -2.5 * np.log10(aconvfnu/3631)
        # aconvflux_errors = np.array([np.sqrt(np.sum(aconv**2 * mask)) for aconv in astropy_errorconvs])
        # i = 1
        # for aconv, aconvflux in zip(astropy_convs, aconvfluxes):
        #     ax = plt.subplot(3, 4, i, projection=wcs)
        #     ax.imshow(aconv, vmin=1e-20, vmax=5e-18, origin='lower',
        #               interpolation='nearest')
        #     ax.set_title(r'%0.4f' % aconvflux)
        #     ax.set_xlabel('RA')
        #     ax.set_ylabel('Dec')
        #     i += 1
        # plt.show()

        fluxes = np.array([np.sum(hdu * mask) for hdu in hdus[1].data])
        # fnu = 3.34e4 * tab['WAVE_EFF']**2 * fluxes * 1e-19
        # magAB = -2.5 * np.log10(fnu/3631)
        flux_errors = np.array([np.sum(np.sqrt(hdu * mask)) for hdu in hdus[2].data])
        # fnu_err = 3.34e4 * tab['WAVE_EFF'] ** 2 * flux_errors * 1e-19
        # mag_err = (2.5 / np.log(10)) * (fnu_err / fnu)
        # print(magAB, mag_err)

        tab.add_columns([fluxes * u.erg / u.cm / u.cm / u.s / u.AA,
                         flux_errors * u.erg / u.cm / u.cm / u.s / u.AA],
                        names=['FLUX', 'FLUX_ERR'])
        # tab.add_columns([magAB, mag_err],
        #                 names=['MAG', 'MAG_ERR'])
    ...
    def make_cubes(self, galdir: str=None, redo: bool=False, dodet: bool=True, get_mask: bool=True,
        ...
        for tile, size in itertools.product(fields, sizes):
            cubename = os.path.join(galdir, "{0}_{1}_{2}x{2}_cube.fits".format(
                galaxy, tile, size))
            ...
            hdus, tab = self.ensable_stamps(galdir=galdir, bands=bands, specz=specz)
            if get_mask and has_errs:
                path2mask: str = os.path.join(galdir, "{0}_{1}_{2}x{2}_{3}.fits".format(
                    galaxy, tile, size, 'mask'))
                if os.path.isfile(path2mask):
                    fitsmask = fits.open(path2mask)
                else:
                    fitsmask = self.calc_masks(galaxy=galaxy, tile=tile, size=size,
                                                savemask=False, savefig=True)
                    ...
                    while mask_sexstars:
                        ...
                        if q1 == 'y':
                            ...
                            fitsmask = self.calc_masks(galaxy=galaxy, tile=tile,
                                                        size=size,
                                                        savemask=False, savefig=True,
                                                        maskstars=maskstars)
                            mask_sexstars = True
                    ...
                    try:
                        ...
                        raise IOError('Something went wrong with the mask')

                hdu3 = fits.ImageHDU(fitsmask[1].data, fitsmask[1].header)
                hdu3.header["EXTNAME"] = ("MASK", "Bitwise mask of the galaxy")
                hdus.append(hdu3)
            elif get_mask and not has_errs:
                raise IOError('Errors needed to calculate masks')
            else:
                raise AssertionError('There is a problem with the masks or weight maps')

            # get galaxy photometry
            tab = self.calculate_galphot(hdus, fitsmask, tab)

            # append metadata table to cube
            thdu = fits.BinTableHDU(tab)
            thdu.header["BSCALE"] = (bscale, "Linear factor in scaling equation")
            thdu.header["EXTNAME"] = "METADATA"
            hdus.append(thdu)
            # add code info to first hdu
            hdus[0].header['CODE'] = (self.code, 'Code used to produce the cubes')
            hdus[0].header['VERSION'] = (self.lastversion, 'Version of the code used to produce the cubes')
            hdus[0].header['MODDATE'] = (self.moddate, 'Date of last modification of the code')
            # save cube
...
if __name__ == "__main__":
    ...
    # scubes.sizes = np.array([900])
    # scubes.angsize = 88.8  # simbad angsize [arcsec]
    # specz = 0.004443
    ...
    # scubes.angsize = 455.5  # simbad angsize [arcsec]
    # specz = 0.004755
    ...
    # scubes.angsize = 113.82  # simbad angsize [arcsec]
    # specz = 0.005070
    ...
    # scubes.angsize = 643.2  # simbad angsize [arcsec]
    # specz = 0.005476

    # # NGC986
    # scubes.galaxies = np.array(['NGC986'])
    # scubes.coords = [['02:33:34.349', '-39:02:42.21']]
    # scubes.tiles = ['SPLUS-s30s23']
    # scubes.sizes = np.array([900])
    # scubes.angsize = 250.2  # simbad angsize [arcsec]
    # specz = 0.006606

    # # NGC1375
    # scubes.galaxies = np.array(['NGC1375'])
    # scubes.coords = [['03 35 16.80', '-35 15 56.379']]
    # scubes.tiles = ['SPLUS-s27s34']
    # scubes.sizes = np.array([900])
    # scubes.angsize = 76.02  # simbad angsize [arcsec]
    # specz = 0.002442

    # NGC7047
    scubes.galaxies = np.array(['NGC7047'])
    scubes.coords = [['21 16 27.643', '-00 49 35.30']]
    scubes.tiles = ['STRIPE82-0113']
    scubes.sizes = np.array([600])
    scubes.angsize = 56.58  # simbad angsize [arcsec]
    specz = 0.01929
