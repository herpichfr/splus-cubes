...
class SCubes(object):
    ...
    def calc_masks(self, galaxy: str=None, tile: str=None, size: int=None,
        ...
        snrs = []
        maskSN = np.zeros(fdata.shape)
        for flux, flux_err in zip(hdus[1].data, hdus[2].data):
            maskSN[flux / flux_err <= 2] = 4
            #snrs.append(flux / flux_err)
        ...
        while increaseCircle:
            ...
            if outerCircRadius >= size / 2:
                raise StopIteration('Radius size exceeds stamp size')
        ...
        fitsmask = f.copy()
        # fmask = mask1 + mask2
        fitsmask[1].data = mask1 + mask2 + maskSN  # np.zeros(maskeddata.shape)
